#!/usr/bin/env python3
"""Generate ORIGINS-COCOA-SCRIPT.md — Master Production & Content Bible."""

from pathlib import Path

OUT = Path(__file__).resolve().parent.parent / "ORIGINS-COCOA-SCRIPT.md"
lines: list[str] = []
...
def write_header() -> None:
    add("""# ORIGINS COCOA — MASTER PRODUCTION & CONTENT BIBLE
...
def write_section_00() -> None:
    add("""## 00. Document Index & How to Use This Guide
...
def write_section_01() -> None:
    add("""## 01. Brand Foundation
...
def write_section_02() -> None:
    add("""## 02. Narrative Architecture (The Story Spine)
...
def write_section_03() -> None:
    """Site Structure & IA — structural centerpiece."""
    add("""## 03. Site Structure & Information Architecture
...
def write_section_04() -> None:
    add("""## 04. Brand Film — Full Director's Treatment
...
def write_section_05() -> None:
    add("""## 05. Page-by-Page Content Specifications
...
def write_section_06() -> None:
    add("""## 06. Voice, Tone & Copy Rules
...
def write_section_07() -> None:
    add("""## 07. Visual & Motion Language
...
def write_section_08() -> None:
    add("""## 08. Production Pipeline (Step-by-Step)
...
def write_section_09() -> None:
    add("## 09. Asset Matrix & Deliverables Registry\n")
    add("### 09.1 Master Asset Table\n")
    headers = ["Asset ID", "Type", "Scene/Page", "Specs", "Filename", "Status", "Owner", "Due"]
    rows = []
    assets = [
    ...
    # Product images 12
    products = [
        "west-godavari-estate", "godavari-reserve", "river-valley-select",
        "prasad-farm-lot", "rao-estate-microlot", "talikadapudi-farm",
        "long-pepper-infusion", "banana-leaf-protocol", "areca-shade-dried",
        "makers-blend-a", "makers-blend-b", "bespoke-lot",
    ]
    for i, slug in enumerate(products, 1):
        rows.append([
            f"OC-P-{i:03d}", "Image", "Products", "3:4 WebP",
            f"product-{slug}-3x4.webp", "Not started", "Photo", "W6"
        ])
    table(headers, rows)


def write_section_10() -> None:
    add("""## 10. Competitive Reference Deep-Dive
...
def write_section_11() -> None:
    add("## 11. Launch Sequence & QA Checklist\n")
    add("### 11.1 Content\n")
    for i, item in enumerate([
        "All page copy matches §05 verbatim", "Farmer names and quotes approved",
        "Product descriptions reviewed by fermentery team", "hello@ and wholesale@ emails active",
        "Social profile URLs live (not #)", "Film captions uploaded", "Alt text on all images",
        "OG images per route", "Favicon SVG deployed", "404 copy implemented",
        "Form success/error states copy", "Press kit PDF (if Phase 2)", "Bean spec sheets PDF",
        "Logo files in brand folder", "Font license confirmed",
    ], 1):
        add(f"- [ ] C-{i:02d}. {item}")
    blank()
    add("### 11.2 Technical\n")
    for i, item in enumerate([
        "Brand film CDN hosted", "Hero film modal functional", "preload=metadata on videos",
        "prefers-reduced-motion honoured", "Lighthouse performance > 85", "Lighthouse accessibility > 90",
        "Mobile Safari video fallback", "All MediaPlaceholder replaced", "SEO title/description per route",
        "Canonical URLs set", "sitemap.xml generated", "robots.txt configured",
        "Analytics installed", "Consent banner (if required)", "HTTPS enforced",
        "Form backend tested", "Internal links verified", "Anchor #traceability works",
        "Image WebP with fallbacks", "Responsive images sizes attr",
    ], 1):
        add(f"- [ ] T-{i:02d}. {item}")
    blank()
    add("### 11.3 Legal\n")
    for i, item in enumerate([
        "Farmer talent releases on file", "Music license documented", "Font license documented",
        "Privacy policy published", "Terms of use (if e-commerce later)", "Cookie policy",
        "GDPR consent if EU traffic expected", "Company registration on footer (if required)",
    ], 1):
        add(f"- [ ] L-{i:02d}. {item}")
    blank()
    add("### 11.4 Marketing\n")
    for i, item in enumerate([
        "Launch date announced internally", "Instagram 5-post film sequence scheduled",
        "LinkedIn founder post drafted", "Partner chocolatier co-announcement",
        "Email to waitlist (if any)", "Press outreach list sent", "Product hunt / industry forums",
        "Google Search Console submitted", "Monitoring uptime alert", "Post-launch analytics review W+1",
    ], 1):
        add(f"- [ ] M-{i:02d}. {item}")
    blank()
    add("### 11.5 Launch Day Sequence\n")
    add("""1. **T-24h** — Staging sign-off, freeze content
...
def write_section_03_expansion() -> None:
    """Additional granular IA specs — expands §03 to production-bible depth."""
    add("### 03.8 Per-Section Component Specification Matrix\n")
    add("Complete field-level specification for every website section. Developers map rows to component props.\n")
    ...
    for route, sections in pages.items():
        add(f"#### Route `{route}` — Section Matrix\n")
        table(
            ["#", "Section", "Component", "Purpose", "Words", "Media", "CTA", "Motion", "Desktop", "Mobile"],
            sections,
        )

    add("""### 03.9 SEO & Metadata Registry (All Routes)
...
def write_section_03_wireframes() -> None:
    add("### 03.16 Wireframe Annotations (Text-Only)\n")
    add("""#### Home `/` — Vertical Scroll Wireframe
...
def write_section_04_expansion() -> None:
    add("### 04.16 Scene Transition Map\n")
    transitions = [
    ...
    table(["Transition", "Type", "Notes"], transitions)

    add("### 04.17 Social Cutdown Specifications\n")
    table(
    ...
    add("### 04.18 Colour Grade Notes Per Scene\n")
    grade_notes = [
    ...
    table(["Scene", "Grade notes"], grade_notes)


def write_section_05_expansion() -> None:
    add("### 05.8 Product Technical Spec Copy (Phase 2 — Maker-Facing)\n")
    add("When spec sheets launch, each product card expands with:\n")
    specs_template = [
        ("Moisture", "7.2% ± 0.3%"),
    ...
    table(["Field", "Example value"], specs_template)

    add("### 05.9 Alternate Headlines (Archived — Not Active)\n")
    table(
    ...
    add("### 05.10 Extended Product Narratives (Maker-Facing)\n")
    add("Long-form descriptions for sample request emails, spec sheets, and future product detail modals.\n")
    products_long = [
    ...
    table(["Product", "Category", "Extended description"], products_long)


def write_section_09_expansion() -> None:
    add("### 09.2 B-Roll to Asset Mapping\n")
    broll_map = [
    ...
    table(["B-Roll ID", "Asset ID", "Output filename"], broll_map)

    add("### 09.3 Status Legend\n")
    table(
        ["Status", "Definition"],
        [
    ...
    # Add more film scene raw assets
    add("### 09.4 Raw Footage Registry (Post-Production)\n")
    headers = ["Clip ID", "Scene", "Description", "Codec", "Duration", "Storage path"]
    rows = []
    for scene in range(1, 9):
        for clip in range(1, 6):
            rows.append([
                f"RAW-S{scene:02d}-C{clip:02d}",
                f"Scene {scene}",
                f"Select {clip} — see shot list §04",
                "ProRes 422 HQ",
                "TBD ingest",
                f"/raw/scene-{scene:02d}/",
            ])
    table(headers, rows)


def write_glossary_expansion() -> None:
    add("### Appendix D.2 Extended Fermentation Vocabulary\n")
    terms = [
...
def write_section_02_expansion() -> None:
    add("### 02.7 Beat-by-Beat Emotional Micro-Map\n")
    beats_detail = [
    ...
    table(["Beat", "Name", "Emotion", "Audio", "Visual", "Web copy anchor"], beats_detail)

    add("### 02.8 Cross-Channel Narrative Consistency\n")
    table(
...
def write_section_10_expansion() -> None:
    add("### 10.10 Page-by-Page Competitive Teardown\n")
    brands = [
    ...
    table(["Brand", "Page type", "Their approach", "OC adoption", "OC differentiation"], brands)

    add("### 10.11 Competitive Film Analysis\n")
    table(
...
def write_section_11_expansion() -> None:
    add("### 11.6 Post-Launch Review Schedule\n")
    table(
    ...
    add("### 11.7 Staging vs Production Checklist\n")
    for env in ["Staging", "Production"]:
        add(f"#### {env}\n")
        items = [
            f"{env}: All routes return 200",
            f"{env}: Film plays in modal",
            f"{env}: Form submits successfully",
            f"{env}: SSL certificate valid",
            f"{env}: Meta tags verified via debugger",
            f"{env}: No MediaPlaceholder visible",
            f"{env}: Footer social links correct",
            f"{env}: Mobile nav functional",
        ]
        for item in items:
            add(f"- [ ] {item}")
        blank()
...
def write_component_handoff() -> None:
    add("### 03.14 Developer Handoff — Component Implementation Notes\n")
    components = [
    ...
    table(["Component", "Implementation note"], components)

    add("### 03.15 Anchor ID Registry\n")
    table(
...
def write_appendix_b_full() -> None:
    add("### Appendix B — Complete Shot List Master\n")
    add("Full shot registry for production. Export to Google Sheets: `OC-ShotList-Master`\n")
    headers = ["Scene", "Shot", "Description", "Type", "Lens", "FPS", "Dur", "Location", "Talent", "Audio", "Status"]
    shots = [
    ...
    add("**Total shots:** 38 primary + pickup coverage (estimate 25% additional B-roll)\n")


def write_appendix_c_full() -> None:
    add("### Appendix C — Full Interview Question Bank\n")
    add("#### C.1 Farmer-Member (30–45 min sit-down or walk-and-talk)\n")
    farmer_qs = [
        "State your name, village, and how long you have farmed cacao.",
        "What crops do you grow alongside cacao? How does intercropping affect your work?",
        "How did you become a farmer-member of Origins Cocoa?",
        "What changed in your harvest and selection practices since joining?",
        "Describe the moment you receive payment — how quickly, and through what method?",
        "What does fair, fast payment mean for your family and your farm investments?",
        "How do you decide which pods are ready for harvest?",
        "What quality standards do you follow that differ from before?",
        "What would you want a chocolate maker in another country to know about your beans?",
        "What is your hope for West Godavari cacao in ten years?",
        "Is there a tradition or story about cacao in your family?",
        "What challenges do you face during monsoon season?",
    ]
    for i, q in enumerate(farmer_qs, 1):
        add(f"{i}. {q}")
    blank()

    add("#### C.2 Fermentery Lead\n")
    ferment_qs = [
        "What does a typical day at the Talikadapudi fermentery look like?",
        "Why was centralising post-harvest at the fermentery necessary?",
        "Explain scientific fermentation in plain language for a maker audience.",
        "How do you decide when to turn the beans?",
        "What data do you record for every batch?",
        "Describe a fermentation trial that surprised you.",
        "How does drying method affect final flavour?",
        "What is the relationship between fermentery team and farmer-members?",
        "What should a new maker understand about Indian cacao's flavour potential?",
        "How do custom fermentation protocols get developed with chocolatiers?",
    ]
    for i, q in enumerate(ferment_qs, 1):
        add(f"{i}. {q}")
    blank()

    add("#### C.3 Founder / Leadership\n")
    founder_qs = [
        "Why West Godavari, and why cacao?",
        "What is broken about how Indian cacao has been treated since the 1960s?",
        "What does Origins Cocoa do differently from commodity traders?",
        "How do you balance investment in fermentery with farmer premiums?",
        "What does success look like in five years?",
        "Who is the maker you most want to reach first?",
        "What will the first bite of an Origins Cocoa bean communicate?",
        "How does traceability technology serve farmers, not just buyers?",
        "What partnership with makers are you most proud of?",
        "What would you say to someone who thinks India cannot produce fine-flavour cacao?",
    ]
    for i, q in enumerate(founder_qs, 1):
        add(f"{i}. {q}")
    blank()

    add("#### C.4 Partner Chocolatier\n")
    choc_qs = [
        "When did you first work with West Godavari beans?",
        "How do these beans behave differently in roasting compared to other origins?",
        "What flavour notes surprised you on first tasting?",
        "How important is lot traceability data to your buying decision?",
        "What would you tell a new maker considering Indian origin for the first time?",
        "Have you collaborated on a custom fermentation protocol? Describe the process.",
        "How do Origins Cocoa beans fit into your product line?",
        "What is the story you tell customers about where the cacao comes from?",
    ]
    for i, q in enumerate(choc_qs, 1):
        add(f"{i}. {q}")
    blank()


def write_section_07_expansion() -> None:
    add("### 07.7 Spacing & Layout Tokens\n")
    table(
        ["Token", "Value", "Usage"],
        [
    ...
    add("### 07.8 Image Aspect Ratio Registry\n")
    table(
    ...
    add("### 07.9 Film Modal Specification (Pending Implementation)\n")
    add("""When `Hero.tsx` Play Film is wired:
...
def write_section_08_weekly_detail() -> None:
    add("### 08.8 Day-by-Day Post-Production Schedule\n")
    days = [
    ...
    add("### 08.9 Equipment List (Minimum Viable)\n")
    table(
...
def write_section_06_expansion() -> None:
    add("### 06.7 Copy Examples by Page Section\n")
    examples = [
    ...
    table(["Section", "Type", "Example", "Why"], examples)

    add("### 06.8 Localization Notes (Future)\n")
    add("""When Telugu or Hindi locales launch:
...
def main() -> None:
    ...
    section_break()
    write_section_02()
    write_section_02_expansion()
    section_break()
    write_section_03()
    write_section_03_expansion()
    write_section_03_wireframes()
    write_component_handoff()
    section_break()
    write_section_04()
    write_section_04_expansion()
    section_break()
    write_section_05()
    write_section_05_expansion()
    section_break()
    write_section_06()
    write_section_06_expansion()
    section_break()
    write_section_07()
    write_section_07_expansion()
    section_break()
    write_section_08()
    write_section_08_weekly_detail()
    section_break()
    write_section_09()
    write_section_09_expansion()
    section_break()
    write_section_10()
    write_section_10_expansion()
    section_break()
    write_section_11()
    write_section_11_expansion()
    section_break()
    write_appendices()
    write_glossary_expansion()
    write_appendix_b_full()
    write_appendix_c_full()
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
